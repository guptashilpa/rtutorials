...
```{r load_data_, echo = FALSE, eval = TRUE}
library(tidyverse)
library(datasets)
indata <- Seatbelts #read.csv("data/nyse_wrr_stocks.csv")
head(indata, 2)

```

```{r compute_covr,echo =TRUE, eval = TRUE}
corMat = cor(indata)
```

```{r compute_eigen,  eval = TRUE}
eigenRes = eigen(corMat)
```

### *Eigenvalues*
```{r print_eigen, exercise = TRUE}
